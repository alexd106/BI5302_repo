...
1\. Create a new R markdown document in your BI5302 RStudio project and save using a suitable file name. I suggest you specify the default output format as pdf but feel free to experiment with html (you can always change this later). Use this R markdown document to record your data exploration, statistical analysis (including graphs and tables) and commentary. For this exercise I would also suggest that you embed your R code as visible chunks within the document (use `echo = TRUE`) for later reference.
...
```{r, q2, collapse=TRUE}
nereis <- read.table("data/Nereis2.txt", header = TRUE)
