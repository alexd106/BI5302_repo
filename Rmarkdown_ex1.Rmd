...
In this exercise you will practice marking up some plain text using R markdown syntax.

Create a new RStudio project called 'BI5302' in RStudio. Now create a new R markdown document and save it with an appropriate file name on your computer (perhaps `squid_analysis.Rmd`). Make sure you fill out the 'Title' and 'Author' information and set the output option to HTML when you create your document. Next, copy and paste the text in the box below into your R markdown document.
...
You will also need to download [this image](images/squid_anatomy.jpg) so you can include it in your final rendered document. Create a subdirectory called `images` in your RStudio project root directory and save the image file there (right click and then choose `save image as...` or similar).
...
Once you're happy with marking up your text, save your R markdown document and then knit your R markdown document to a HTML file. Open this file in your web browser by clicking on the squid_analysis.html file in the 'Files' pane in RStudio and select the Open in browser option. Check the formatting is correct. You could also try knitting to a pdf document. 
...
## Sampling and data description
These data were originally collected as part of a study published in Aquatic Living Resources (Smith *et al*.,
2005). The aim of the study was to investigate the seasonal patterns of investment in somatic and 
reproductive tissues in the long finned squid *Loligo forbesi* caught in Scottish waters. Squid 
were caught monthly from December 1989 - July 1991 (**month** and **year variables**). After capture, 
each squid was given a unique specimen code (**specimen variable**) and the following variables measured: 
