...
```{r, bonus_rms, fig.height=5.5, fig.width=5.5, fig.align="center", message=FALSE }
library(rms)
...
w.oahu <- as.data.frame(rms::Predict(f, 
                     ...
                     max(dataf$rainfall[dataf$location == "Oahu"]), 
                     length = 50), 
                     location = "Oahu"))

pred_dat <- rbind(w.maui, w.oahu)
ggplot(pred_dat, aes(x = rainfall, y = yhat, group = location, fill = location)) +
  ...
  geom_point(data = dataf, 
             aes(x = rainfall, y = abund, group = location, colour = location)) +
  labs(x = "rainfall" , 
       y = "bird abundance")
