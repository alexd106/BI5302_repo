...
There doesn't appear to be any obvious outliers in the `abund` variable for each island. However this plot does indicate that the 'spread' of bird abundance values is much greater for Oahu than for Maui. Hopefully, our linear model will be anle to account for this.  
...
From the Residual vs Fitted and Scale-Location plots (left-hand side) there doesn't seem to be any issues with heterogeneity of variance. The QQ-plot suggests that the residuals are also approximately normally distributed. The residuals vs leverage plot indicates that non of our residuals are unusual or influential. Looks good so far!

However, let's be really thorough and plot our residuals against each explanatory variable. First we can plot the residuals from our linear model against the `location` variable. 
